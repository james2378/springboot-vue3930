<template>
	<div class="diy_details_box diy_div_order_center">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "order_center_id",
					source_id: this.obj["order_center_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/order_center/set?order_center_id=' + _this.obj["order_center_id"], {
								praise_len
							}, (res) => {
								if(res.result){

<template>
	<div class="diy_details_box diy_div_user_registration">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "user_registration_id",
					source_id: this.obj["user_registration_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/user_registration/set?user_registration_id=' + _this.obj["user_registration_id"], {
								praise_len
							}, (res) => {
								if(res.result){

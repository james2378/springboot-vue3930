<template>
	<div class="diy_details_box diy_div_movie_attendance">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "movie_attendance_id",
					source_id: this.obj["movie_attendance_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/movie_attendance/set?movie_attendance_id=' + _this.obj["movie_attendance_id"], {
								praise_len
							}, (res) => {
								if(res.result){

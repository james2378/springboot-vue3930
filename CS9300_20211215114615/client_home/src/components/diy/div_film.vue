<template>
	<div class="diy_details_box diy_div_film">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
						...
						<!-- 图片 -->
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/order_center/edit','add')" @click="to_form('/order_center/edit')" >购买 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/the_latest_movie/edit','add')" @click="to_form('/the_latest_movie/edit')" >上新 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/film/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/film/set?film_id=' + _this.obj["film_id"], {
								praise_len
							}, (res) => {
								if(res.result){

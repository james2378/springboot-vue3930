<template>
	<div class="diy_details page_the_latest_movie" id="the_latest_movie_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_the_latest_movie'>
							<div_the_latest_movie :obj="obj"></div_the_latest_movie>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_the_latest_movie from "@/components/diy/div_the_latest_movie.vue";
	import mixin from "@/mixins/page.js";
	export default {
		mixins:[mixin],
		components:{
			div_the_latest_movie
		},
		data(){
			return{
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/the_latest_movie/set?the_latest_movie_id=' + obj["the_latest_movie_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							...
							source_field: "the_latest_movie_id",
							source_id: this.obj.the_latest_movie_id,
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "the_latest_movie",
					source_field: "the_latest_movie_id",
					source_id: obj["the_latest_movie_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/the_latest_movie/set?the_latest_movie_id=' + _this.obj["the_latest_movie_id"], {
								praise_len
							}, (res) => {
								if(res.result){

		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/the_latest_movie/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_the_latest_movie = !show_list_the_latest_movie">
		                        <div class="switch_box">
		                            <span v-if="show_list_the_latest_movie"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--最新电影推荐列表组件 -->
		                <list_the_latest_movie v-if="show_list_the_latest_movie" :list="list_the_latest_movie" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_the_latest_movie.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_the_latest_movie" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_the_latest_movie" :key="i" @click="to_details('the_latest_movie',o,'the_latest_movie_id')">
		                            <td v-for="(oj,n) in fields_the_latest_movie">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/film/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_film = !show_list_film">
		                        <div class="switch_box">
		                            <span v-if="show_list_film"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--电影推荐列表组件 -->
		                <list_film v-if="show_list_film" :list="list_film" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_film.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_film" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_film" :key="i" @click="to_details('film',o,'film_id')">
		                            <td v-for="(oj,n) in fields_film">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
										<span v-else-if="oj.type && oj.type == 'UID' ">
											<span style="display: none">
...
<script>
	...
	import list_film from "@/components/diy/list_film.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_the_latest_movie: true,
				list_the_latest_movie: [],
				vm_arr_the_latest_movie: [
					"the_latest_movie_id",
					"movie_title",
					"movie_number",
					"film_classification",
		...
		methods: {
			...
			// 获取最新电影列表
			get_the_latest_movie() {
				let url = "~/api/the_latest_movie/get_list?";
				url = "~/api/the_latest_movie/get_list?orderby=hits desc";
				this.$get(url, {
					...
					if (json.result) {
						this.list_the_latest_movie = json.result.list;
					}
				})
			},
			// 获取电影列表
			get_film() {
				let url = "~/api/film/get_list?";
				url = "~/api/film/get_list?orderby=hits desc";
				this.$get(url, {
					"orderby": "`recommend` desc",
					"page": 1,
					"size": 8
					...
					if (json.result) {
						this.list_film = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_the_latest_movie() {
		        var list = this.list_the_latest_movie;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_the_latest_movie.map((o) => {
		                // 第二个中括号是对象的属性
		    ...
		    list_table_film() {
		        var list = this.list_film;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_film.map((o) => {
		                // 第二个中括号是对象的属性
		                list_table[i][o] = list[i][o] || "";
		            });
		        }

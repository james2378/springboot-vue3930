<template>
	<el-main class="bg">
			...
			<el-row>
				...
				<el-col :xs="24" :sm="24" :lg="8">
					<el-form-item label="场次">
						<el-input v-model="query.number_of_sessions"></el-input>
					</el-form-item>
				</el-col>
				<el-col :xs="24" :sm="24" :lg="8">
					<el-form-item label="电影名称">
						<el-input v-model="query.movie_title"></el-input>
					</el-form-item>
				</el-col>
				<el-col :xs="24" :sm="24" :lg="8">
					<el-form-item label="购买人">
						<el-select v-model="query.purchaser">
							<el-option v-for="o in list_purchaser" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
									   :value="o['user_id']">
							</el-option>
						</el-select>
					</el-form-item>
				...
				<el-col :xs="24" :sm="10" :lg="8">
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/order_center/table','add') || $check_action('/order_center/view','add')" class="el-button el-button--default el-button--primary" to="./view?">添加
						</router-link>
						<el-button v-if="user_group == '管理员' || $check_action('/order_center/table','del') || $check_action('/order_center/view','del')" class="" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="number_of_sessions" label="场次"
				v-if="user_group == '管理员' || $check_field('get','number_of_sessions')" min-width="200">
			</el-table-column>
			<el-table-column prop="price" label="价格"
				v-if="user_group == '管理员' || $check_field('get','price')" min-width="200">
			</el-table-column>
			<el-table-column prop="movie_number" label="电影编号"
				v-if="user_group == '管理员' || $check_field('get','movie_number')" min-width="200">
			</el-table-column>
			<el-table-column prop="movie_title" label="电影名称"
				v-if="user_group == '管理员' || $check_field('get','movie_title')" min-width="200">
			</el-table-column>
			<el-table-column prop="number_of_purchases" label="购买数"
				v-if="user_group == '管理员' || $check_field('get','number_of_purchases')" min-width="200">
			</el-table-column>
			<el-table-column prop="total" label="合计"
				v-if="user_group == '管理员' || $check_field('get','total')" min-width="200">
			</el-table-column>
			<el-table-column prop="purchaser" label="购买人"
				v-if="user_group == '管理员' || $check_field('get','purchaser')" min-width="200">
				<template slot-scope="scope">
					{{ get_user_purchaser(scope.row['purchaser']) }}
				</template>
			</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/order_center/table','set') || $check_action('/order_center/view','set') || $check_action('/order_center/view','get') || $check_action('/取消/table','add') || $check_action('/取消/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/order_center/table','set') || $check_action('/order_center/view','set') || $check_action('/order_center/view','get')"
					...
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/order_center/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/order_center/get_list?like=0",
				url_del: "~/api/order_center/del?",
		...
		methods: {
			...
			openPayModal(v){
				this.pay_obj.id = v.order_center_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/order_center/set?order_center_id="+this.pay_obj.id);

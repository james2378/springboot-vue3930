		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['user_registration_id'] && $check_field('set','gender')) || (!form_sub['user_registration_id'] && $check_field('add','gender'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','date_of_birth') || $check_field('add','date_of_birth') || $check_field('set','date_of_birth')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="出生日期" prop="date_of_birth">
					<el-input id="date_of_birth" v-model="form_sub['date_of_birth']" placeholder="请输入出生日期"
							  v-if="user_group === '管理员' || (form_sub['user_registration_id'] && $check_field('set','date_of_birth')) || (!form_sub['user_registration_id'] && $check_field('add','date_of_birth'))"></el-input>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["user_registration_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?user_registration_id=" + form_sub["user_registration_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/user_registration/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/user_registration/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/user_registration/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/user_registration/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/user_registration/view','get');
